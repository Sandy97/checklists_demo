...
begin
  ...
  fdqGroups.Close;
  try
    fdqGroups.SQL.Text := 'SELECT T.GRID,T.GR_PARENT,T.GR_NAME_REF,T.GR_DESCRIPTION FROM GROUPS T';
    fdqGroups.Open;
    fdqGroups.SaveToStream(ms, TFDStorageFormat.sfJSON);
    //fdqGroups.SaveToFile('c:\temp\fdqGroups.json',sfJSON);
...
begin
  ms := TMemoryStream.Create;
  try
    fdqGroups.SQL.Text := 'SELECT T.GRID,T.GR_PARENT,T.GR_NAME_REF,T.GR_DESCRIPTION FROM GROUPS T where T.GRID=:g_id';
